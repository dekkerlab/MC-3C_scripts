...
# Identify fully mapped pacbio fragments
pacbio_frag_fully_mapped <- direct_ints %>%
  group_by(pacbio_frag_ID) %>%
  summarise(fully_mapped = !("*" %in% V1 | "*" %in% V4))

# Merge the fully mapped information with the interaction dataframe
direct_ints <- merge(direct_ints, pacbio_frag_fully_mapped)

# Subset the interaction data frame to get only those from fully mapped pacbio fragments
direct_ints <- subset(direct_ints, fully_mapped == TRUE)

# Remove any interactions that are not mapped LEGACY
direct_ints <- subset(direct_ints, (V1!="*" | V4!="*") & (V1!="*") & (V4!="*"))

# Remove any self-interactions
direct_ints <- subset(direct_ints, !(V1 == V4 & (V2==V5 | V3==V6)))
...
direct_ints[direct_ints$V1 != direct_ints$V4, "dist"] <- NA
...
unique_reduced_ranges <- sapply(fragment_rangeslist, function(x) length(GenomicRanges::reduce(x)))
...
for (chr in chrs) {
  ...
  hela_chr_positive_bins <- with(subset(hela_chr_compartments,
                                        eigen1 > 0),
                                 GenomicRanges::reduce(GRanges(seqnames = X.chr,
...
direct_ints_first_frag_ranges <- with(direct_ints,
                                      GRanges(seqnames = V1,
...
direct_ints[first_frag_compartment_overlaps$queryHits,
...
direct_ints_second_frag_ranges <- with(direct_ints,
                                       GRanges(seqnames = V4,
...
direct_ints[second_frag_compartment_overlaps$queryHits,
...
for (chr in chrs) {
  ...
  hela_chr_tads <- GenomicRanges::reduce(resize(hela_chr_tads, 40001))
...
class_1_ranges <- with(subset(steps_per_frag, class == 1),
...
direct_ints[first_frag_tad_overlaps$queryHits,
...
direct_ints[second_frag_tad_overlaps$queryHits,
...
save(direct_ints, steps_per_frag,
     ...
     pairwise_intrachr,
     file = "frames_for_plotting_stricter.RData")
